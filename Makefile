...
ifeq (cleveldb,$(findstring cleveldb,$(DHUB_BUILD_OPTIONS)))
  build_tags += gcc
else ifeq (rocksdb,$(findstring rocksdb,$(DHUB_BUILD_OPTIONS)))
  build_tags += gcc
endif
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=dhub \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=dhubd \
...
ifeq (,$(findstring nostrip,$(DHUB_BUILD_OPTIONS)))
  ldflags += -w -s
endif
...
draw-deps:
	...
	@goviz -i ./cmd/dhubd -d 2 | dot -Tpng -o dependency-graph.png
...
docker-build-debug:
	@docker build -t dhub:debug --build-arg BASE_IMG_TAG=debug -f Dockerfile .
...
localnet-keys:
	. tests/localdhub/keys.sh

localnet-build:
	@docker build -t local:dhub -f tests/localdhub/Dockerfile .

localnet-build-state-export:
	@docker build -t local:dhub-se --build-arg ID=$(ID) -f tests/localdhub/mainnet_state/Dockerfile-stateExport .

localnet-start:
	@docker-compose -f tests/localdhub/docker-compose.yml up

localnet-start-state-export:
	@docker-compose -f tests/localdhub/mainnet_state/docker-compose-state-export.yml up

localnet-stop:
	@docker-compose -f tests/localdhub/docker-compose.yml down

localnet-remove: localnet-stop
	PWD=$(shell pwd)
	@docker run --user root -v ${PWD}/tests/localdhub/.dhubd:/root/dhub ubuntu /bin/sh -c "rm -rf /root/dhub/*"

localnet-remove-state-export:
	@docker-compose -f tests/localdhub/mainnet_state/docker-compose-state-export.yml down
